	...
	: rowIDs(0), colIDs(0), maxCols(0), HI(0), maxLvl(0), sampleNameSep(""), doSubsets(false), doHigh(false)
{
	...
	if (isGZfile(inF)) {
#ifdef _gzipread
		in = new igzstream(inF.c_str(), ios::in); cout << "Reading gzip input\n";
#else
		cout << "gzip not supported in your rtk build\n"; exit(50);
#endif
	}
...
Matrix::Matrix(options* opts, const string xtra, vector<string>& outFName,
	...
	doSubsets(false), doHigh(highLvl)
{
	...
	out.close();*/
	#ifdef notRpackage
	std::cout << "Read " << geneCnt << " rows and " << cnt << " columns\n";
	#endif
}
...
Matrix::Matrix(options* opts, const string xtra, bool highLvl)
	: rowIDs(0), colIDs(0), maxCols(0), HI(0), maxLvl(0), sampleNameSep(""), 
	doSubsets(false), doHigh(highLvl), sparse(opts->sparse)
{
	const string inF = opts->input;
	//reads matrix from HDD
...
//	ifstream in(inF.c_str());
	...
	while (safeGetline((*in), line)) {
		...
		geneCnt++;
		if (false) {//single core
			cntNA = processLine2Mat1(line, cntNA, geneCnt, ini_ColPerRow);
		}
		else { // multi core
			if (usedThred) {
				cntNA = job.get();
			}
			job = async(std::launch::async, &Matrix::processLine2Mat1, this, line, cntNA, geneCnt, ini_ColPerRow);
			usedThred = true; 
		}
	}
	if (usedThred) {
...
int Matrix::processLine2Mat1(string  line, int cntNA, 
		int geneCnt, int ini_ColPerRow){
	string segments;
	string rowID = "";
	stringstream ss;
	ss << line;
	...
	while (getline(ss, segments, '\t')) {
		...
		if (cnt2 == -1) {
			...
			curRowIdx = rowIDs.size();
			rowID_hash[rowID] = curRowIdx;
			rowIDs.push_back(rowID);
	...
	for (uint i = 0; i < tmpV.size(); i++) {
		if (sparse) {
			if (tmpV[i] > 1e-15) {
				matSp[i][curRowIdx] = tmpV[i];
			}
		}
		else {
			mat[i].push_back(tmpV[i]);
		}
		colSum[i] += (double)tmpV[i];
	}
...
bool Matrix::check_compli(int x, int y, const float thresh, const float minp) {
...
void Matrix::complimentarity(vector<string>& cand, vector<float>& id, options* opt,
	...
	float thresh = 0.9f; float minpval = opt->pval;
	...
	for (uint i = 0; i < cand.size(); i++) {
		...
		for (uint j = i+1; j < cand.size(); j++) {
			...
			bool isCompl = check_compli(idx1[i], idx1[j], thresh, minpval);
			if ( (i==0 && id[j]>0.99)
				|| isCompl) {
				//curClus += "," + cand[j]; 
...
void Matrix::decluter(options* opts) {
	...
	while (true) {
		...
		int nsize = clbl->txt.size() - 1;
		vector<string> curClus(nsize, "");
		vector<float> clusIDs(nsize, 0.f);
		getGenesInClstr(clbl, curClus, clusIDs);
		if (curClus.size() < 2) {
			delete clbl;
			if (!clbl->cont) { break; }
			continue;
...
void Matrix::decluter2(options* opts) {
	...
	list.close();
	delete fet;
	std::cout << "Decluter2:\nMerged " << totalMergedID << "/" << totalMergedCE << "/" << cases << " cases\nWriting new matrix to " << outF2 << endl;
...
void Matrix::writeMatrix(const string of, bool onlyFilled) {
	...
	*out << endl;
	vector<mat_fl> rowSums;
	size_t cidS(colIDs.size());
	future<string> job; bool j_use(false);
	string tmp;
	...
	for (size_t i = 0; i<rowIDs.size(); i++) {
		...
		job = async(std::launch::async, &Matrix::row2line, this, i);
		j_use = true;
		//tmp = row2line(i);
...
:LvlName(L), FeatureNs(Features), SampleNs(Samples),mat(0), catCnt(0), hiTaNAcnt(0){
	empty = vector<mat_fl>(SampleNs.size(), 0);
	mat.resize(FeatureNs.size(), empty);
	catCnt.resize(FeatureNs.size(),0);
...
void HMat::print(ofstream& O,bool mean){
	O << LvlName ;
